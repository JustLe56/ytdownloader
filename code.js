			...
			{ 
				...
				for (i = 0; i <= numChapters; i++){
					...
					ffmpeg('raw_download_out.mp4')
						...
						.withVideoCodec('copy') //mp4 specific
						.withAudioCodec('copy') //mp4 specific
						.output('video_'+i+'.mp4') //mp4 specific
						// .noVideo() //mp3 specific 
						// .format(`mp3`) //mp3 specific 
						//.output('video_'+i+'.mp3') //mp3 specific 
						.on('end', function(err) {
							if(!err) { 
								...
								if (conversionCount === numChapters){
									for (var j = 0; j <= numChapters; j++){
										zip.addLocalFile("video_"+j+".mp4"); //check type before running
									}
									var zipFileContents = zip.toBuffer();
									
									//clean up local files
									for (var j = 0; j <= numChapters; j++){
										fs.unlink("video_"+j+".mp4", (err) =>{ //check type before running
											if(err)throw err;
